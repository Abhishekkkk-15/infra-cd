...
func GithubWebhook(c *gin.Context) {
	...
	err = db.DB.
		...
		First(&webhook).Error
	if err != nil {
		// No matching project — silently accept (return 200 to avoid GitHub retries)
		c.JSON(http.StatusOK, gin.H{"status": "no matching project"})
		return
	}

	// Verify HMAC signature
	sig := c.GetHeader("X-Hub-Signature-256")
	if !verifyGithubSignature(webhook.Secret, body, sig) {
		c.JSON(http.StatusUnauthorized, gin.H{"error": "invalid signature"})
		return
	}

	// Trigger deployment
	if _, err := services.TriggerDeployment(webhook.ProjectID); err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
		return
	}

	c.JSON(http.StatusOK, gin.H{"status": "deployment triggered"})
}
